...
def to_batch(
...
) -> tuple[Batch, list[dict[UUIDKey[ModularRobot], MultiBodySystem]], list[dict[UUIDKey[ModularRobot]], BodyToMultiBodySystemMapping]]:
    """
    ...
    batch.scenes.extend(simulation_scene for simulation_scene, _, _ in converted)

    return batch, [multibodymap for _, multibodymap, _ in converted], [bodymap for _, _, bodymap in converted]
