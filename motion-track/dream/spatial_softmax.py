...
class SoftArgmaxPavlo(torch.nn.Module):
    def __init__(self, n_keypoints=5, learned_beta=False, initial_beta=25.0):
        ...
        if learned_beta:
            ...
            self.beta = (torch.ones(n_keypoints) * initial_beta).to(torch.device('cuda'))
    ...
    def forward(self, heatmaps, size_mult=1.0):
        ...
        col_idx = col_repeat.view(1, 1, n_row, n_col).to(torch.device('cuda'))
        ...
        row_idx = row_repeat.view(1, 1, n_row, n_col).to(torch.device('cuda'))
