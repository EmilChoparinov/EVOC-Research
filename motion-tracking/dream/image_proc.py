...
def overlay_points_on_image(
    ...
    for idx_point in range(n_image_points):
        ...
        if image_point_names:
            ...
            cv2.putText(
                img=drawn_image,
                text=image_point_names[idx_point],
                org=text_position,
                fontFace=cv2.FONT_HERSHEY_SIMPLEX,
                fontScale=font_scale,
                color=annot_color_text,
...
def combine_belief_maps(map1, map2):
...
def create_individual_belief_map(
    image_resolution,
    # image size (width x height)
    pointBelief,
    # one point to draw
    sigma=2
    # the size of the point
    # returns a tensor of h x w with the belief maps
):
    ...
    pixel_u = int(pointBelief[0])
    pixel_v = int(pointBelief[1])
    array = np.zeros(image_transpose_resolution)
...
def jules_peaks_from_belief_maps(belief_map_tensor, threshold=0.1):
    peaks_batch = []
    for j in range(belief_map_tensor.size()[0]):
    # Find local maxima in the heatmap
        belief_map = np.array(belief_map_tensor[j].cpu())

        local_max = (belief_map == maximum_filter(belief_map, size=6))
        foreground = (belief_map>threshold)
        # eroded_background = binary_erosion(background, structure=neighborhood, border_value=1)
        detected_peaks = np.logical_and(local_max,foreground)

        peaks_positions = np.nonzero(detected_peaks == True)
        peaks_postion_tuples = []
        for pos_index in range(0,len(peaks_positions[0])):
            peaks_postion_tuples.append((
                peaks_positions[1][pos_index], peaks_positions[0][pos_index]
                ))
        peaks_batch.append(peaks_postion_tuples)
    return peaks_batch

def jules_peaks_from_belief_maps_np_array_single(belief_map_np_array, threshold=0.1):
    peaks_batch = []
    local_max = (belief_map_np_array == maximum_filter(belief_map_np_array, size=6))
    foreground = (belief_map_np_array>threshold)
    # eroded_background = binary_erosion(background, structure=neighborhood, border_value=1)
    detected_peaks = np.logical_and(local_max,foreground)
