...
if __name__ == "__main__":
    ...
    for n in range(1, 7):
        ...
        y = net(torch.zeros(batch_size, 3, 400, 400).to(torch.device('mps')))
        print(y[-1].shape)
        print()
        del net, y
    ...
    print("ResnetSimple")
    net = ResnetSimple(n_keypoints).to(torch.device('mps'))
    y = net(net_input.to(torch.device('mps')))
    print(y[-1].shape)
    print()
    del net, y

    print("ResnetSimpleFull")
    net = ResnetSimple(n_keypoints, full=True).to(torch.device('mps'))
    y = net(net_input.to(torch.device('mps')))
    print(y[-1].shape)
    print()
    del net, y
    ...
    print("DOPE")
    net = DopeNetworkBelief().to(torch.device('mps'))
    y = net(net_input.to(torch.device('mps')))
    print(y[-1].shape)
    print()
    del net, y
    ...
    y = net1(net_input.to(torch.device('mps')))
    print(y[-1].shape)
    print()
    del net1, y
    ...
    y = net3(net_input.to(torch.device('mps')))
    print(y[-1].shape)
    print()
    del net3, y
    ...
    y = net5(net_input.to(torch.device('mps')))
    print(y[-1].shape)
    print()
    del net5, y
    ...
    y = net7(net_input.to(torch.device('mps')))
    print(y[-1].shape)
    print()
    del net7, y
