...
def load_keypoints(data_path, object_name, keypoint_names):
    ...
    # trouble! make this a dictionary that tracks keypoint name?
    for kp_name in keypoint_names:
        ...
        # )
        if kp_name not in object_keypoint_names:
            # there is no joint/box in the image - so add None
            keypoint_data["positions_wrt_cam"].append(None)
            keypoint_data["projections"].append(None)
        else:
