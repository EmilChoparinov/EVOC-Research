...
def scene_to_model(
    ...
    # Mapping for rigid bodies similar to how it's done for joints
    for mbs_i, rigid_bodies_and_names in enumerate(all_rigid_bodies_and_names):
        for rigid_body, name in rigid_bodies_and_names:
           
            # Hack to adjust the names...
            # Format :: msbx_link0_...linkN -> msbx/msbx_link0_..._linkN
            # Format :: msbx -> msbx/
            name_fix = name.replace('_', f"/mbs{mbs_i}_", 1)
            
            if name == f'mbs{mbs_i}':
                name_fix = f'{name}/'
                
            name_fix = name_fix.rstrip('_')

            # Retrieve the MuJoCo body ID using the exact name
            mujoco_body_id = model.body(name_fix).id
            
            # Create a mapping entry for this rigid body
            mapping.rigid_body[UUIDKey(rigid_body)] = RigidBodyMujoco(id=mujoco_body_id)
