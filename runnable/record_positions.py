...
csv_map = {                       # Positions within `body` to record into CSV
    "head": body.core_v2,
...
# === Pure functional environment to perform simulation ========================
...
import pdb
def record_into_csv(csv_id: int, simulation: tuple[list[SceneSimulationState], ModularRobot]):
    ...
    df.to_csv(f"./../out/record_positions/run-{csv_id}.csv", index=False)
# ==============================================================================
...
for run_id in range(num_itrs):
    record_into_csv(run_id, perform_simulation(run_id))    
