...
def translation_rotation_csv(df: pd.DataFrame) -> pd.DataFrame:
    # Initialize a list to hold transformed points
    transformed_data = []

    # Extract initial head and forward coordinates, parsing from strings
    head_first = ast.literal_eval(df['head'].iloc[0])
    forward_first = ast.literal_eval(df['forward'].iloc[0]) if 'forward' in df.columns else (0, 0)

    # Calculate rotation angle based on the initial forward direction
    theta = (3* np.pi / 2) - np.arctan2(forward_first[1] - head_first[1], forward_first[0] - head_first[0])
    ...
    # Define body parts to transform
    body_parts = ['head', 'middle', 'rear', 'right_front', 'left_front', 'right_hind', 'left_hind']

    # Process each row
    for index, row in df.iterrows():
        transformed_points = {
            'robot_index': row.get('generation_id', np.nan),
            'frame_id': row.get('frame_id', np.nan)
        }

        # Parse and transform each body part's coordinates
        for part in body_parts:
            if part in row and pd.notna(row[part]):
                try:
                    # Parse the coordinate string to a tuple
                    coord = ast.literal_eval(row[part])
                    if isinstance(coord, tuple) and len(coord) == 2:
                        # Translate by subtracting the initial head position
                        translated_coord = np.array(coord) - np.array(head_first)
                        # Rotate the translated point
                        rotated_coord = rotation_matrix @ translated_coord
                        # Store the transformed point as a formatted string
                        transformed_points[part] = f"({rotated_coord[0]:.2f}, {rotated_coord[1]:.2f})"
                    else:
                        print(f"Invalid format for {part} in row {index}")
                except (ValueError, SyntaxError) as e:
                    print(f"Error parsing coordinates for {part} in row {index}: {e}")
            else:
                transformed_points[part] = None

        # Append the transformed points dictionary to the list
        transformed_data.append(transformed_points)
...
def size_scaling_csv(df: pd.DataFrame) -> pd.DataFrame:
    def parse_tuple_string(s):
        """Safely parse coordinate strings to tuples."""
        return ast.literal_eval(s) if pd.notna(s) else (None, None)

    global_max_distance_animal = 168.13387523042465
    global_max_distance_robot = 0  # Initialize for finding maximum distance across all frames
    coordinates_list = []

    # First, determine the maximum distance across all frames
    for index, frame in df.iterrows():
        coordinates = {
            'head': parse_tuple_string(frame.get('head', None)),
            'middle': parse_tuple_string(frame.get('middle', None)),
        ...
        if None in coordinates.values() or any(v is None for v in coordinates.values()):
            print(f'Frame {index}: Incomplete robot coordinates: {coordinates}')

        # Append coordinates for later scaling
        coordinates_list.append(coordinates)

        # Calculate distances from head to other body parts
        if coordinates['head'] is not None:
            robot_head = np.array(coordinates['head']).astype(float)
            robot_boxes = np.array([coordinates[part] for part in
                                    ['middle', 'rear', 'right_front', 'left_front', 'right_hind', 'left_hind']]).astype(float)
            robot_distances = np.linalg.norm(robot_boxes - robot_head, axis=1)
            max_distance_robot = np.max(robot_distances)

            # Update the global maximum distance if this frame's distance is larger
    ...
    # Scale coordinates for each frame based on the global maximum distance
    scaled_robot_data = []
    first_robot_head = np.array(coordinates_list[0]['head']).astype(float).reshape(1, 2)

    for i, coordinates in enumerate(coordinates_list):
        # Extract and scale coordinates
        robot_coordinates = np.array([coordinates[part] for part in
                                      ['head', 'middle', 'rear', 'right_front', 'left_front', 'right_hind', 'left_hind']]).astype(float)
        scaled_robot_coordinates = (robot_coordinates - first_robot_head) * (global_max_distance_animal / global_max_distance_robot) + first_robot_head
