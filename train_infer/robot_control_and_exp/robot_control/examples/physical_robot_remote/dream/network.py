...
class DreamNetwork:
    ...
    def loss(self, network_input_heads, target):
        ...
        if self.network_config["architecture"]["output_heads"] == ["belief_maps"]:
            ...
            if "n_stages" in self.network_config["architecture"]:
                ...
                # loss = torch.tensor(0).float().to(torch.device('mps'))
                # for belief in network_output_heads: loss += ((belief - target) * (belief - target)).mean()

            else:
                # Jules: This is what gets called in basic example
    ...
    def keypoints_from_image(
        ...
        converted_detected_kp_projs_net_out_batch = []

        for keypoints_per_limb in detected_kp_projs_net_out_batch[0]:
            key_points_array = []
            for kp in keypoints_per_limb:
                key_points_array.append((int(kp[0]), int(kp[1])))
            converted_detected_kp_projs_net_out_batch.append(key_points_array)

        # Convert keypoints from net-output to net-input
        net_in_keypoints_per_type = []
        for keypoints_per_limb in converted_detected_kp_projs_net_out_batch:
            detected_kp_projs_net_in = dream.image_proc.convert_keypoints_to_netin_from_netout(
                keypoints_per_limb, netout_res_inf, netin_res_inf
            )
            net_in_keypoints_per_type.append(detected_kp_projs_net_in)

        detected_kp_projs_per_type = []
        for net_in_key_points in net_in_keypoints_per_type:
            ...
            detected_kp_projs_per_type.append(detected_kp_projs)


        detection_result = {"detected_keypoints": detected_kp_projs_per_type}
        if debug:
            ...
            detection_result["detected_keypoints_net_input"] = net_in_keypoints_per_type
    ...
    def inference(self, network_input):
        ...
        if self.architecture_type == "dream_legacy":
            ...
            # belief_map_1 = dream.image_proc.image_from_belief_map(
            #     belief_maps_batch[0][0]
            # )
            # belief_map_2 = dream.image_proc.image_from_belief_map(
            #     belief_maps_batch[0][1]
            # )
            # belief_map_1.save("beliefmap1.png")
            # belief_map_2.save("beliefmap2.png")
            # print("saved maps as beliefmap1.png and beliefmap2.png")
            ...
            for belief_maps in belief_maps_batch:
                peaks_batch_jules_test = dream.image_proc.jules_peaks_from_belief_maps(
                    belief_maps#, **peaks_from_belief_maps_kwargs
                )
                peaks_batch = dream.image_proc.peaks_from_belief_maps(
                    belief_maps, **peaks_from_belief_maps_kwargs
                )

                # Determine keypoints from this
                # detected_kp_projs = []
                # for peaks in peaks_batch:
                #     detected_kp_projs.append(peaks)
                #     if len(peak) == 1:
                ...
                # detected_kp_projs_batch.append(peaks_batch)
                detected_kp_projs_batch.append(peaks_batch_jules_test)
