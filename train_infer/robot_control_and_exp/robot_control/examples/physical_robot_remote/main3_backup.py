...
def record(stop_event, time_stamp):
    ...
    out = cv2.VideoWriter('/Users/xyh/Thesis/code/revolve2/examples/physical_robot_remote/evaluate_100_iter_6/'+video_name, fourcc, 20.0, size)
    # global record_flag
...
def main(weights, bound) -> None:
    ...
    # weights_path = '/Users/xyh/Thesis/code/revolve2/examples/physical_robot_remote/evaluate_100_iter_6/'+weights_name
    ...
    video_name = 'evaluate_video_' + time_stamp + '.mp4'
    video_path = '/Users/xyh/Thesis/code/revolve2/examples/physical_robot_remote/evaluate_100_iter_6/'+video_name
    return video_path
...
for i in range(1, iter):
    # decrease the scale from 0.2 to 0.01 after 100 iterations
    scales.append(scales[-1]-(0.2-0.01)/iter)
bound = 2.0
...
if __name__ == "__main__":
    ...
    if os.path.exists('/Users/xyh/Thesis/code/revolve2/examples/physical_robot_remote/evaluate_100_iter_6/saved_weights.json'):
        with open('/Users/xyh/Thesis/code/revolve2/examples/physical_robot_remote/evaluate_100_iter_6/saved_weights.json', 'r') as f:
            data = json.load(f)
            ...
            index = int(data['iter'])
            weight_tmp = data['weights_pick_log'][-1] if 'weights_pick_log' in data else np.zeros(9) + np.random.normal(0, scales[index], 9)
    else:
        weight_tmp = np.zeros(9) + np.random.normal(0, scales[0], 9)
        max_xy_dis_log = []
        xy_dis_log = []
        weights_log = []
        video_log = []
    ...
    while index < iter:
        ...
        if len(weights_pick_log) > 0:
            print("last weights picked: ", weights_pick_log[-1])
        else:
            weights_pick_log.append(weights_list[0])
        noise = np.random.normal(0, scales[index], 9)
        print("noise: ", noise)
        weights_list = [
        ...
        print("$$$$$$$$======iter: ", index, " ======$$$$$$$$")
        print("current scale: ", scales[index])
        ...
        with open('/Users/xyh/Thesis/code/revolve2/examples/physical_robot_remote/evaluate_100_iter_6/saved_weights.json', 'w') as f:
            json.dump(dumped, f)
