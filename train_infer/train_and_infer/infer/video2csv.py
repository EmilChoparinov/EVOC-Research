...
def network_inference(args):
    ...
    if not capture.isOpened():
        print(f"Error: Could not open video file {args.image_path}. Please check the file path and format.")
    #用于保存从视频每帧提取的关键点坐标
    csv_file_path = os.path.join(args.save_dir, 'gecko_data.csv')
    ...
    with open(csv_file_path, 'w', newline='') as csvfile:
        ...
        while True:
            ...
            #kp_coords_wrtNetInput_asArray 是一个二维列表，其中的每一项包含某一类型的关键点（如头部、框、关节）坐标
            ...
            # # head = np.array(head_data[0])
            # head=head_data[0]
            # head = head_data[0] if head_data else (0, 0)

            head = np.array(head_data[0]) if head_data.size > 0 else np.array([0, 0])
...
if __name__ == "__main__":
    args = Arguments()
    print(f"Video path: {args.image_path}")
