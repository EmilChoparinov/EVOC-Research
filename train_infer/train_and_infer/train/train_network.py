...
def train_network(args):
    ...
    if args.resume_training:
        ...
        best_valid_network_config_path = os.path.join(
            args.output_dir, "best_network_2.yaml"
        )
...
# !!TODO: Update find_ndds_data_in_dir !!
    ...
    print("Found data config: {}".format(found_data_config))
    ...
    # Nvidia TODO: possibly read in other arguments here, such as optimizer, instead of using command line defaults
    ...
    # honestly not sure what is going on here but this is passed in args.not_augment_data
    ...
    # a nice dictionary to store all the configuration.
    # TODO needs to be updated according to any added arguments. E.g. allow_multiple_peaks?
    ...
    if args.resume_training:
        ...
        # #needs to be consistent of course if one is resuming training
        network_config = most_recent_network_config_file

        print("~~ RESUMING TRAINING FROM {} ~~".format(most_recent_epoch_weight_path))
        print("")

    # if not resuming training start_epoch is 0
    else:
        start_epoch = 0

    # Print to screen
    print("Network configuration: {}".format(network_config))
    # TODO: here the network gets initiated! update this stuff to handle adjustments

    dream_network = dream.create_network_from_config_data(network_config)
    if args.resume_training:
        # some extras that need to be done to resume training
        dream_network.model.load_state_dict(
            torch.load(os.path.join(args.output_dir, most_recent_epoch_weight_path))
        )

    # this needs to be done to initialise the optimizer
    #启用模型的训练模式 以确保模型的参数可以被优化
    dream_network.enable_training()

    # The following ensures the config is consistent with the dataloader
    # it seems the output resolution gets calulated through the model which is good because
    # then no change needed - except the config file in the train data (TODO)
    (
        trained_net_input_res,
        trained_net_output_res,     # image_raw_resolution gets derived from training data: not 
                                    # manip_config nor arch_conifg
    ...
    # Create NDDS dataset and loader                                    set to NONE when training
    training_debug_mode = dream.datasets.ManipulatorNDDSDatasetDebugLevels["INTERACTIVE"]
    ...
    found_dataset = dream.datasets.ManipulatorNDDSDataset(
        ...
        trained_net_input_res,
        trained_net_output_res,
        dream_network.image_normalization, # comes from arch_configs
    ...
    for e in tqdm(range(start_epoch, args.epochs)):
        ...
        for batch_idx, sample in enumerate(tqdm(train_data_loader)):
            ...
            network_input_heads = []
            network_input_heads.append(sample["image_rgb_input"].to(device))

            if dream_network.network_config["architecture"]["target"] == "belief_maps":
                training_labels = sample["belief_maps"].to(device)
            elif dream_network.network_config["architecture"]["target"] == "keypoints":
                training_labels = sample["keypoint_projections_output"].to(device)
            else:
                assert (
                    False
        ...
        with torch.no_grad():
            ...
            for valid_batch_idx, valid_sample in enumerate(tqdm(valid_data_loader)):
                ...
                valid_network_input_heads.append(valid_sample["image_rgb_input"].to(device))

                if (
                    dream_network.network_config["architecture"]["target"]
                    == "belief_maps"
                ):
                    valid_labels = valid_sample["belief_maps"].to(device)
                elif (
                    dream_network.network_config["architecture"]["target"]
                    == "keypoints"
                ):
                    valid_labels = valid_sample["keypoint_projections_output"].to(device)
                else:
                    assert (
                        False
        ...
        if mean_valid_loss_per_batch < best_valid_loss:
            ...
            if save_results:
                dream_network.save_network(
                    args.output_dir, "best_network_2", overwrite=True
                )
